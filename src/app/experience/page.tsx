...
const groupProjectsByRole = (projects: ProjectExperience[]) => {
  return projects.reduce<Record<string, ProjectExperience[]>>((acc, project) => {
    if (!acc[project.role]) {
      acc[project.role] = [];
    }
    ...
    <div className="container mx-auto px-4 py-8 md:py-20">
      ...
      <div className="mb-8 md:mb-12 max-w-3xl">
        ...
        <p className="mt-3 md:mt-4 text-base md:text-lg text-muted-foreground">
          A detailed timeline of my professional journey, roles, and key contributions at each
          company.
        </p>
      </div>
      ...
      >
        ...
        {experiences.map((experience, index) => {
          const groupedProjects = groupProjectsByRole(experience.projects);

          return (
            <TimelineItem key={experience.id} isActive={activeItemId === experience.id}>
              <div
                onMouseEnter={() => setActiveItemId(experience.id)}
                ...
                <Card className="shadow-sm bg-card/90 backdrop-blur-sm transition-shadow duration-300 hover:shadow-md">
                  <CardHeader className="px-4 sm:px-6 py-4 sm:py-6">
                    <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-start">
                      <div>
                        <CardTitle className="text-lg md:text-xl">{experience.company}</CardTitle>
                  ...
                  <CardContent className="space-y-4 md:space-y-6 px-4 sm:px-6 pb-4 sm:pb-6">
                    ...
                    {(experience.achievements || experience.technologies) &&
                      experience.projects.length > 0 && <hr className="border-border/50" />}
                    ...
                    {experience.projects.length > 0 && (
                      <div>
                        ...
                        <Accordion
                          ...
                          className="w-full"
                        >
                          {Object.entries(groupedProjects).map(([role, projectsInRole]) => (
                            <ProjectSection
                              key={role}
                              role={role}
                              projectsInRole={projectsInRole}
                            />
                          ))}
                        </Accordion>
                      </div>
